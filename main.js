...
scene.add(camera);
gltfLoader.load("assets/cobolt.glb", obj => {
    let weapon = obj.scene;
...
let entities = [];
gltfLoader.load("assets/ant.glb", obj => {
    ...
    for (let i = 0; i < 12; i++) {
        ...
        const ant = new Ant(obj.scene, obj.animations, {
...
gltfLoader.load("assets/stinger.glb", stinger => {
    stinger.scene.children[0].material.envMap = textures.envMap;
    gltfLoader.load("assets/bee.glb", obj => {
        obj.scene.traverse((child) => {
            if (child.isMesh) {
                child.material.envMap = textures.envMap;
            }
        ...
        for (let i = 0; i < 5; i++) {
            const chosenSpot = possibleSpots[Math.floor(Math.random() * possibleSpots.length)];
            const bee = new Bee(obj.scene, obj.animations, {
                position: new THREE.Vector3(chosenSpot.x * 5, 0, chosenSpot.y * 5),
                scene,
                tileMap,
                heightMap,
                ...
                entities,
                playerController,
                projectileMesh: stinger.scene.children[0],
                direction: 2 * Math.PI * Math.random()
            });
...
gltfLoader.load("assets/station.glb", obj => {
    obj.scene.traverse((child) => {
        if (child.isMesh) {
            child.material.envMap = textures.envMap;
        }
    ...
    const station = new Station(obj.scene, obj.animations, {
...
gltfLoader.load("assets/lever.glb", obj => {
    obj.scene.traverse((child) => {
        if (child.isMesh) {
            child.material.envMap = textures.envMap;
        }
    ...
    for (let i = 0; i < 4; i++) {
        ...
        const chosenSpot = possibleSpots[Math.floor(Math.random() * possibleSpots.length)];
        const lever = new Lever(obj.scene, obj.animations, {
            position: new THREE.Vector3(chosenSpot.x * 5, 0, chosenSpot.y * 5),
            camera,
            scene,
            entities
