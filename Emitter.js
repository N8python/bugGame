...
class Emitter extends THREE.Object3D {
    ...
    update(delta, camera) {
        const timeScale = (delta * 1000) / 16.6666;
        ...
        for (let i = 0; i < this.particles.length; i++) {
            ...
            if (particle.colorDecay !== undefined) {
                particle.color.r *= particle.colorDecay;
                particle.color.g *= particle.colorDecay;
                particle.color.b *= particle.colorDecay;
            }
            if (particle.velocityDecay !== undefined) {
                particle.velocity.position.multiplyScalar(particle.velocityDecay);
                particle.velocity.rotation.multiplyScalar(particle.velocityDecay);
                particle.velocity.scale.multiplyScalar(particle.velocityDecay);
            }
